...
export async function sdkGeneratorLogic(
  ...
  for (const [modelName, { fields }] of Object.entries(models)) {
    ...
    dependencies.generateFiles(
      tree,
      dependencies.joinPathFragments(__dirname, './graphql'),
      modelDir,
      { className, propertyName, pluralClassName, pluralPropertyName, fragmentFields, kebabName, tmpl: '' },
    );
  ...
  dependencies.generateFiles(
    tree,
    dependencies.joinPathFragments(__dirname, './files'),
    sdkSrcDir,
    { tmpl: '' },
  );
