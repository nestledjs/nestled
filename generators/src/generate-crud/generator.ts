...
async function createLibraries(tree: Tree, overwrite = true) {
  ...
  // Create feature library for generated-crud
...
async function generateModelFiles(tree: Tree, dataAccessLibraryRoot: string, featureLibraryRoot: string, models: { name: string; fields: readonly { name: string; type: string }[]; primaryField: string; modelName: string; modelPropertyName: string; pluralModelPropertyName: string }[], name = 'generated-crud', overwrite = true) {
...
`;
  ...
  // Always write feature module file
  tree.write(
    joinPathFragments(featureLibraryRoot, 'src/lib/api-crud-feature.module.ts'),
    featureModuleContent
  );

  // Create index.ts file for feature library
  const featureIndexContent = `export * from './lib/api-crud-feature.module'
${models.map(model => `export * from './lib/${toKebabCase(model.modelName)}.resolver'`).join('\n')}
`;

  // Always write feature index file
  tree.write(
    joinPathFragments(featureLibraryRoot, 'src/index.ts'),
    featureIndexContent
  );

  // Generate individual resolver files for each model in feature library
  for (const model of models) {
    const resolverFilePath = joinPathFragments(featureLibraryRoot, `src/lib/${toKebabCase(model.modelName)}.resolver.ts`);
...
export default async function(tree: Tree, schema: GenerateCrudGeneratorSchema) {
  try {
    ...
    const { dataAccessLibraryRoot, featureLibraryRoot } = await createLibraries(tree, true)

    // Generate model files with the provided name or default to 'generated-crud'
    await generateModelFiles(tree, dataAccessLibraryRoot, featureLibraryRoot, models, schema.name || 'generated-crud', true)
