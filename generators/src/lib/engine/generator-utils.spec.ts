...
describe('generator-utils', () => {
  ...
  describe('readPrismaSchema', () => {
    ...
    it('should read and combine prisma schema files from a directory in a deterministic (sorted) order', () => {
      // Written user-first, post-second, but concatenation is sorted alphabetically
      // (post before user) so the order is stable across platforms/filesystems.
      tree.write('prisma/user.prisma', 'model User {}')
      tree.write('prisma/post.prisma', 'model Post {}')
      const expectedContent = 'model Post {}\nmodel User {}\n'
  ...
  describe('getAllPrismaModels', () => {
    ...
    describe('generated CRUD security boundary', () => {
      ...
      it('does not emit obsolete per-model auth metadata', async () => {
        const models = await loadModels([{ name: 'User', fields: [] }])

        expect(models[0]).not.toHaveProperty('auth')
        expect(generateDatabaseModelContent(models)).not.toContain('auth?:')
      })

      it('rejects legacy @crudAuth instead of silently ignoring it', async () => {
        await expect(
          loadModels([{ name: 'User', fields: [], documentation: '@crudAuth: { "readMany": "user" }' }]),
        ).rejects.toThrow(/Remove @crudAuth from: User/)
      })
    })
  })
