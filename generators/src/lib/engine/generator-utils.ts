...
export function readPrismaSchema(tree: Tree, prismaPath: string) {
  ...
  if (isDirectory) {
    ...
    // Sort for a deterministic concatenation order across platforms — tree.children()
    // wraps readdirSync, whose order is filesystem-dependent (alphabetical on macOS/APFS,
    // arbitrary on Linux/ext4). Sorting keeps the emitted model order stable everywhere,
    // so regenerating on CI doesn't churn the generated files.
    const schemaFiles = tree
      .children(prismaPath)
...
export function getNpmScope(tree: Tree): string {
  ...
  return match[1] // Returns just "nestled" from "@nestledjs/source"
}
...
/**
 * Shared `@skipCrud` model/relation filtering — the single source of truth used by
 * every Prisma-driven generator (crud, custom, sdk, models) so they can never
 * disagree about which models exist or which relation fields survive.
 */
export function isSkipCrudModel(model: { documentation?: string }): boolean {
  return Boolean(model.documentation?.includes('@skipCrud'))
}

export function filterSkippedRelationFields<T extends { kind: string; type: string }>(
  fields: readonly T[],
...
/**
 * Generates TypeScript content for database model information.
 * This creates a file with interfaces, constants, and helper functions
 * for accessing Prisma model metadata in a type-safe way.
 */
export function generateDatabaseModelContent(models: ModelType[]): string {
  return `// Auto-generated database model information
...
export function getDatabaseModelField(modelName: string, fieldName: string): DatabaseField | undefined {
