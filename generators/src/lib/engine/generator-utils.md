...
- Retrieves information about all Prisma models, including their fields and primary field.
- Rejects legacy `@crudAuth` annotations because generated CRUD is always admin-only.

## `assertNoCrudAuthAnnotations(models)`

- Fails generation with the annotated model names and the explicit-resolver migration path.
