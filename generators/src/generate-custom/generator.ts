...
async function generateCustomFiles(tree: Tree, dataAccessLibraryRoot: string, featureLibraryRoot: string, models: ModelType[], npmScope: string, name: string) {
...
${models.map((model) => `import { ${model.modelName}Service } from './${toKebabCase(model.modelName)}.service'`).join('\n')}
