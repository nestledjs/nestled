...
exports.initConfigGenerator = initConfigGenerator;
...
function handleEnvExample(tree) {
    ...
    if (!tree.exists(envPath) && tree.exists(envExamplePath)) {
        ...
        devkit_1.logger.info('✅ Created .env from .env.example');
    }
}
...
function handleDockerFilesAndScripts(tree) {
    ...
    if (tree.exists(packageJsonPath)) {
        ...
        devkit_1.logger.info('✅ Added Docker scripts to package.json');
    }
}
function ensureEnvInGitignore(tree) {
    ...
    if (tree.exists(gitignorePath)) {
        ...
        if (!gitignoreContent.includes('.env')) {
            ...
            devkit_1.logger.info('✅ Added .env to .gitignore');
        }
        else {
            devkit_1.logger.info('ℹ️  .env already exists in .gitignore');
        }
    }
    else {
        tree.write(gitignorePath, '.env\n');
        devkit_1.logger.info('✅ Created .gitignore and added .env');
    }
}
function addNxScriptsToPackageJson(tree) {
    const nxScripts = {
    ...
    if (tree.exists(packageJsonPath)) {
        const packageJsonContent = JSON.parse(tree.read(packageJsonPath, 'utf-8') || '{}');
        packageJsonContent.scripts = Object.assign(Object.assign({}, packageJsonContent.scripts), nxScripts);
        tree.write(packageJsonPath, JSON.stringify(packageJsonContent, null, 2));
        devkit_1.logger.info('✅ Added Nx-related scripts to package.json');
    }
}
function initConfigGenerator(tree) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        (0, devkit_1.addDependenciesToPackageJson)(tree, { '@prisma/internals': '^5.14.0' }, { yaml: '^2.4.2' });
        ...
        (0, utils_1.addScriptToPackageJson)(tree, 'clean', 'git reset --hard HEAD && git clean -fd && rm -rf node_modules && rm -rf tmp && rm -rf dist && rm -rf apps && rm -rf libs && pnpm install');
        // Create or update pnpm-workspace.yaml
        (0, utils_1.updatePnpmWorkspaceConfig)(tree, { packages: ['apps/**', 'libs/**', 'tools/*'] });
...
exports.default = initConfigGenerator;
