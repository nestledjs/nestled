...
exports.removeWorkspacesFromPackageJson = removeWorkspacesFromPackageJson;
exports.addScriptToPackageJson = addScriptToPackageJson;
exports.deleteFiles = deleteFiles;
exports.deleteDirectory = deleteDirectory;
exports.endsWithQuestionMark = endsWithQuestionMark;
exports.removeQuestionMarkAtEnd = removeQuestionMarkAtEnd;
exports.getPrismaSchemaPath = getPrismaSchemaPath;
exports.readPrismaSchema = readPrismaSchema;
exports.mapPrismaTypeToNestJsType = mapPrismaTypeToNestJsType;
...
exports.updateTypeScriptConfigs = updateTypeScriptConfigs;
exports.getAllPrismaModels = getAllPrismaModels;
exports.updatePnpmWorkspaceConfig = updatePnpmWorkspaceConfig;
exports.pnpmInstallCallback = pnpmInstallCallback;
exports.addToModules = addToModules;
exports.apiLibraryGenerator = apiLibraryGenerator;
...
function removeWorkspacesFromPackageJson(tree) {
...
function addScriptToPackageJson(tree, scriptName, scriptCommand) {
    const packageJsonPath = 'package.json';
    if (tree.exists(packageJsonPath)) {
        (0, devkit_1.updateJson)(tree, packageJsonPath, (json) => {
            var _a;
            json.scripts = (_a = json.scripts) !== null && _a !== void 0 ? _a : {};
            json.scripts[scriptName] = scriptCommand;
            return json;
        });
    }
}
function deleteFiles(tree, filesToDelete) {
...
function deleteDirectory(tree, dirPath) {
    if (tree.exists(dirPath)) {
        tree.children(dirPath).forEach((child) => {
            const childPath = (0, devkit_1.joinPathFragments)(dirPath, child);
...
function removeQuestionMarkAtEnd(str) {
    return str.replace(/\?$/, '');
}
function getPrismaSchemaPath(tree) {
    var _a;
    ...
    return ((_a = packageJson.prisma) === null || _a === void 0 ? void 0 : _a.schema) || null;
}
function readPrismaSchema(tree, prismaPath) {
    if (!prismaPath) {
...
function mapPrismaTypeToNestJsType(prismaType) {
    const typeMap = {
        String: 'string',
        Boolean: 'boolean',
        Int: 'number',
...
function parsePrismaSchema(schemaContent, modelName) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        try {
            const dmmf = yield (0, internals_1.getDMMF)({ datamodel: schemaContent });
...
function generateTemplateFiles({ tree, schema, libraryRoot, templatePath, npmScope, }) {
    const variables = Object.assign(Object.assign(Object.assign({}, schema), (0, devkit_1.names)(`${schema.name}`)), { npmScope, tmpl: '' });
    (0, devkit_1.generateFiles)(tree, templatePath, libraryRoot, variables);
}
/**
...
function installPlugins(tree_1) {
    return tslib_1.__awaiter(this, arguments, void 0, function* (tree, dependencies = {}, devDependencies = {}, _options = {}) {
        ...
        if (Object.keys(depsToInstall).length || Object.keys(devDepsToInstall).length) {
            return (0, devkit_1.addDependenciesToPackageJson)(tree, depsToInstall, devDepsToInstall);
        }
 ...
 */
function updateTsConfigPaths(tree, importPath, libraryRoot) {
...
function updateTypeScriptConfigs(tree, libraryRoot) {
    const updateTsConfigForProject = (projectRoot) => {
    ...
    for (const dir of projectDirs) {
        if (tree.exists(dir)) {
            for (const project of tree.children(dir)) {
                const projectPath = (0, devkit_1.joinPathFragments)(dir, project);
...
function getAllPrismaModels(tree) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const { getDMMF } = yield import('@prisma/internals');
        const { getPrismaSchemaPath, readPrismaSchema } = yield import('./generator-utils.js');
        const pluralize = (yield import('pluralize')).default;
        ...
        try {
            const dmmf = yield getDMMF({ datamodel: prismaSchema });
...
function updatePnpmWorkspaceConfig(tree, options) {
    var _a, _b;
    ...
    if ((_a = options.packages) === null || _a === void 0 ? void 0 : _a.length) {
        let packagesSeq = doc.get('packages');
        if (!packagesSeq) {
    ...
    if ((_b = options.onlyBuiltDependencies) === null || _b === void 0 ? void 0 : _b.length) {
        let onlyBuiltSeq = doc.get('onlyBuiltDependencies');
        if (!onlyBuiltSeq) {
...
function addToModules({ tree, modulePath, moduleArrayName, moduleToAdd, importPath }) {
    var _a;
    ...
    let fileContent = ((_a = tree.read(modulePath)) === null || _a === void 0 ? void 0 : _a.toString()) || '';
...
function apiLibraryGenerator(tree, schema, templateRootPath, type, addModuleImport) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const npmScope = getNpmScope(tree);
        const API_LIBS_SCOPE = 'libs/api';
        const libraryRoot = type
            ? (0, devkit_1.joinPathFragments)(API_LIBS_SCOPE, schema.name, type)
            : (0, devkit_1.joinPathFragments)(API_LIBS_SCOPE, schema.name);
        ...
        if (schema.overwrite && tree.exists(libraryRoot)) {
            try {
                (0, child_process_1.execSync)(`nx g rm ${libraryName} --forceRemove`, {
                    stdio: 'inherit',
                    cwd: tree.root,
                });
            }
        ...
        if (!tree.exists(API_LIBS_SCOPE)) {
            tree.write((0, devkit_1.joinPathFragments)(API_LIBS_SCOPE, '.gitkeep'), '');
        }
        ...
        if (type) {
            finalTemplatePath = (0, devkit_1.joinPathFragments)(templateRootPath, type);
        }
        else {
        ...
        if (addModuleImport) {
            const nameClassName = (0, devkit_1.names)(schema.name).className;
            const typeClassName = type ? (0, devkit_1.names)(type).className : '';
